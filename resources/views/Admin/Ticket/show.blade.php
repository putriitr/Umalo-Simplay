        ...
        <div class="card-body">
            ...
            @if($ticket->status == 'open' && $ticket->jenis_layanan == 'Permintaan Data')
                ...
                <form action="{{ route('admin.tickets.process', $ticket->id_after_sales) }}" method="POST"
                    ...
                    <div class="form-group mb-3">
                        <label for="file_pendukung_layanan"><i class="fas fa-upload me-2"></i>Unggah Data Pendukung
                            (Opsional)</label>
                        <input type="file" name="file_pendukung_layanan" id="file_pendukung_layanan" class="form-control">
                    </div>
                    <button type="submit" class="btn btn-primary"><i class="fas fa-check-circle me-2"></i>Proses dan
            ...
            @elseif($ticket->status == 'open' && in_array($ticket->jenis_layanan, ['Visit', 'Maintanance', 'Installasi']))
                ...
                <form action="{{ route('admin.tickets.process', $ticket->id_after_sales) }}" method="POST" class="mt-4">
                    ...
                    @method('PUT')
                    <div class="form-group mb-3">
                        <label for="keterangan_tindakan"><i class="fas fa-clipboard-list me-2"></i>Keterangan Penjadwalan
                            {{ $ticket->jenis_layanan }}</label>
                        <textarea name="keterangan_tindakan" id="keterangan_tindakan" class="form-control" rows="4"
                            placeholder="Deskripsikan penjadwalan..."></textarea>
                    </div>
                    <div class="form-group mb-3">
                    ...
                    <button type="submit" class="btn btn-primary"><i class="fas fa-calendar-check me-2"></i>Proses dan
            ...
            @elseif($ticket->status == 'progress' && in_array($ticket->jenis_layanan, ['Visit', 'Maintanance', 'Installasi']))
                ...
                <form action="{{ route('admin.tickets.complete', $ticket->id_after_sales) }}" method="POST"
                    ...
                    @method('PUT')
                    <div class="form-group mb-3">
                        <label for="file_pendukung_layanan"><i class="fas fa-upload me-2"></i>Unggah Arsip Data Pendukung
                            (Opsional)</label>
                        <input type="file" name="file_pendukung_layanan" id="file_pendukung_layanan" class="form-control">
                    </div>
                    <button type="submit" class="btn btn-success"><i class="fas fa-check-double me-2"></i>Selesaikan
