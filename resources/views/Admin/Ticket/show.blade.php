        ...
        <div class="col-md-12">
            <div class="card p-3 shadow">
                ...
                <div class="card mt-4 shadow">
                    <div class="card-body">
                        <div class="card-body">
                            <p><strong><i class="fas fa-cogs me-2"></i>Jenis Layanan:</strong>
                                {{ $ticket->jenis_layanan }}</p>
                            <p><strong><i class="fas fa-info-circle me-2"></i>Keterangan Pengajuan:</strong>
                                {{ $ticket->keterangan_layanan }}</p>
                            <p><strong><i class="fas fa-calendar-alt me-2"></i>Tanggal Pengajuan:</strong>
                    ...
                    @else bg-secondary @endif">
                            ...
                            @if ($ticket->dok_pendukung_tindakan)
                                <p><a href="{{ asset($ticket->dok_pendukung_tindakan) }}" target="_blank"
                                        class="text-primary"><i class="fas fa-file-alt me-2"></i>Lihat Dokumen Tindakan</a>
                                </p>
                            @else
                            ...
                            @if($ticket->status == 'open' && $ticket->jenis_layanan == 'Permintaan Data')
                                ...
                                <form action="{{ route('admin.tickets.process', $ticket->id_after_sales) }}" method="POST"
                                    enctype="multipart/form-data" class="mt-4">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group mb-3">
                                        <label for="file_pendukung_layanan"><i class="fas fa-upload me-2"></i>Unggah Data
                                            Pendukung
                                            (Opsional)</label>
                                    ...
                                    <button type="submit" class="btn btn-primary"><i
                                            class="fas fa-check-circle me-2"></i>Proses dan
                                        Selesaikan Tiket</button>
                                </form>
                            @elseif($ticket->status == 'open' && in_array($ticket->jenis_layanan, ['Visit', 'Maintanance', 'Installasi']))
                                ...
                                <form action="{{ route('admin.tickets.process', $ticket->id_after_sales) }}" method="POST"
                                    ...
                                    @method('PUT')
                                    <div class="form-group mb-3">
                                        <label for="keterangan_tindakan"><i
                                                class="fas fa-clipboard-list me-2"></i>Keterangan
                                            Penjadwalan
                                            {{ $ticket->jenis_layanan }}</label>
                                        <textarea name="keterangan_tindakan" id="keterangan_tindakan" class="form-control"
                                            rows="4" placeholder="Deskripsikan penjadwalan..."></textarea>
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="tim_teknis"><i class="fas fa-users me-2"></i>Tim Teknis</label>
                                        <input type="text" name="tim_teknis" id="tim_teknis" class="form-control"
                                            placeholder="Masukkan nama tim teknis">
                                    </div>
                                    <button type="submit" class="btn btn-primary"><i
                                            class="fas fa-calendar-check me-2"></i>Proses
                                        dan
                                        Jadwalkan {{ $ticket->jenis_layanan }}</button>
                                </form>
                            @elseif($ticket->status == 'progress' && in_array($ticket->jenis_layanan, ['Visit', 'Maintanance', 'Installasi']))
                                ...
                                <form action="{{ route('admin.tickets.complete', $ticket->id_after_sales) }}" method="POST"
                                    enctype="multipart/form-data" class="mt-4">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group mb-3">
                                        <label for="file_pendukung_layanan"><i class="fas fa-upload me-2"></i>Unggah Arsip
                                            Data
                                            Pendukung
                                            (Opsional)</label>
                                        <input type="file" name="file_pendukung_layanan" id="file_pendukung_layanan"
                                    ...
                                    <button type="submit" class="btn btn-success"><i
                                            class="fas fa-check-double me-2"></i>Selesaikan
                                        {{ $ticket->jenis_layanan }} dan Arsipkan</button>
                                </form>
                            @endif
                            <div class="d-flex justify-content-end mt-4">
                                <a href="{{ route('admin.tickets.index') }}" class="btn btn-secondary"><i
